...
class Index extends Component {
  ...
  handleChange = (event) => {
    console.log(event.target.name, "000", event.target.value)
    let { postData } = this.state
  ...
  uploadFile = async (e) => {
    ...
    console.log("OPOPOPP", response)
    if (response) {
      postData["imageUrl"] = response.path
      this.setState({ postData, isFileSelected: response.filename })
    }
  ...
  handleSubmit = async () => {
    ...
    if (response) {
      alert("Thank You for Visit and Adding a Post!")
      setTimeout(()=>{
  ...
  render() {
    let { isFileSelected, postData } = this.state
    return (
      <div>
